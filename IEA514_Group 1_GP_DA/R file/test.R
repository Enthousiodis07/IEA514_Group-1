#### Import the data ####
library(readxl)
G1_data <- read_excel("data/IEA514_Group 1.xlsx")
View(G1_data)
...
# Pre-eliminary data processing
row_odd <- seq_len(nrow(G1_data)) %% 2 # odd-even row adjustment
G1_data_a <- G1_data[row_odd == 1, ] # Group a
G1_data_b <- G1_data[row_odd == 0, ] # Group b

# Group a
Japan_a <- G1_data_a[c(1:15),]
Pakistan_a <- G1_data_a[c(16:23),]
Thai_a <- G1_data_a[c(24:31),]
Group_1a <- G1_data_a[32,]

# Group b
Japan_b <- G1_data_b[c(1:15),]
Pakistan_b <- G1_data_b[c(16:23),]
Thai_b <- G1_data_b[c(24:31),]
Group_1b <- G1_data_b[32,]



samples_a <- c(rep('Japan_a', 15), rep('Pakistan_a', 8), rep('Thai_a', 8), 
             rep ('Group 1_a', 1))

samples_b <- c(rep('Japan_b', 15), rep('Pakistan_b', 8), rep('Thai_b', 8), 
               rep ('Group 1_b', 1))

# Change the column sample from being a character to a factor

G1_data$Sample <- as.factor(G1_data$Sample)
G1_data_a$Sample <- as.factor(G1_data_a$Sample)
G1_data_b$Sample <- as.factor(G1_data_b$Sample)

#### Na ####
# Scatter plot
# Graphic device jpeg
jpeg('figs/scatter_Na.jpeg', res = 300, units = 'cm', height = 8, width = 8)
# The margins of the plot: bottom, left, top, right
par(mar = c(4,4,1,0.5))
plot(G1_data_a$Na_mg.kg, pch = 19, xaxt = 'n', # Disable the x-axis labels to use custom names
     type = 'o',
     xlab = 'Location', 
     ylab = "Na [unknown]",
     col = as.factor(samples_a))

axis(1, at = 1:nrow(G1_data_a), labels = samples_a) # Custom axis labels
legend('topleft', c('Japan_a','Pakistan_a','Thai_a', 'Group 1a'), pch = c(19,19,19,19),
       col = c('yellow','red','green','black'))
dev.off()
...
G1.pca <- prcomp(G1_data[,-1], # Remove the first column
                 center = TRUE, # Center the data, similar to scaling
                 scale. = TRUE) # Need to be scaled because of magnitude diff.

jpeg('figs/biplot_G1.jpeg', res = 300, units = 'cm', height = 16, width = 16)
# The margins of the plot: bottom, left, top, right
par(mar = c(4,4,1,1))
plot(G1.pca$x[,1:2], # Plot only the first and second PCs
     pch = 20,        # The marker 
     col = G1_data$Sample, # The color
     xlim = c(-5,10), ylim = c(-2.5,6)) # The limits of the axes
dataEllipse(G1.pca$x[,1],      # PC1
            G1.pca$x[,2],      # PC2
            groups = G1_data$Sample, # Groups
            lwd = 1,            # line width
            group.labels = c('Japan','Pakistan','Thai'), 
...
legend('bottomright', c('Japan','Pakistan','Thai'), pch = c(20,20,20),
       col = c('black', 'red', 'green'))
dev.off()
